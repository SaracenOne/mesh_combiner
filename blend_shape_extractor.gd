...
static func insert_blend_vertex(p_blend_verticies : Dictionary, p_vertex_index : int, p_array_index : int, p_new_value : Vector3, p_format : int) -> Dictionary:
...
static func extract_blend_shape_from_mesh_combiner(p_mesh_combiner : MeshCombiner, p_blend_shape_name : String) -> BlendShapeData:
				...
				for array_index in range(0, blend_shape_arrays.size()):
					...
					if array_index == Mesh.ARRAY_VERTEX or array_index == Mesh.ARRAY_NORMAL:
						if (base_array[vertex_index].distance_to(blend_shape_array[vertex_index]) > 0.00001):
							...
							var insert_blend_vertex_ret : Dictionary = insert_blend_vertex(blend_verticies, vertex_index, array_index, new_value, format)
